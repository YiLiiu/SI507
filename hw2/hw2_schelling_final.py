...
class PurpleAgents(AgentNew):
    def __init__(self, xlocation, ylocation, group="Purple", status="unhappy"):
        AgentNew.__init__(self, xlocation, ylocation, group, status)
...
class AgentNew(Agent):
    ...
    def check_neighbors(self, agentlist):  # note this method needs not only the self attributes - it also needs a list of agents
        ...
        same_group_neighbor = [x for x in zlist if x.group in self.group] ### use list comprehension to only keep members of zlist who are of the same group as this agent
        opposite_group_neighbor = [x for x in zlist if x.group not in self.group]### use list comprehension to only keep members of zlist who are of the opposite group as this agent
...
class PurpleDiversitySeekers(AgentNew):
    ...
    def check_neighbors(self, agentlist): 
        ...
        zlist = list(filter(lambda agent : abs(agent.x-self.x) < 10, agentlist))   ### here, use a filter and a lambda function to find all agents in a list who are within 10 spaces in the X direction of the agent that is calling the method
        zlist = list(filter(lambda agent : abs(agent.y-self.y) < 10, zlist))### do the same thing now to filter for agents within 10 spaces in the y direction - what list do you want to apply this lambda function to?
        same_group_neighbor = [x for x in zlist if x.group in self.group]### use list comprehension to only keep members of zlist who are of the same group as this agent
        opposite_group_neighbor = [x for x in zlist if x.group not in self.group]### use list comprehension to only keep members of zlist who are of the opposite group as this agent
